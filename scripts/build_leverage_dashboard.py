...
讀 data/leverage/ 本地庫 + src.leverage 的 DPI，畫成內嵌 SVG 圖表，
無外部相依、深/淺色自適應，可直接放上 GitHub Pages。
用法：python scripts/build_leverage_dashboard.py
"""
import json
import sys
...
from src.leverage import compute_dpi, dpi_level, load_market  # noqa: E402
...
def line_chart(dates, values, w=680, h=220, color="#3b82f6", fill=True,
               reflines=None, y_fmt=lambda v: f"{v:,.0f}", value_fmt=None):
    """回傳一段 <svg> 折線圖字串（含 y 軸 min/mid/max、x 軸首末日、格線、參考線）。"""
...
def main():
    ...
    bx_vals = [r["buxian_total_kshares"] / 1e5 for r in bx_market]  # 仟股→億股：×1000股 ÷1e8
    ...
    # 大盤指標卡數據
    bal_chg = pct(bal_yi[0], bal_yi[-1])
    maint_chg = maint[-1] - maint[0]
    bx_chg = pct(bx_vals[0], bx_vals[-1])
  ...
  <section class="hero">
    <div class="gauge lv-{last['dpi']//15 if last['dpi']<60 else 4:.0f}">
      <div class="dpi-num">{last['dpi']:.0f}</div>
      <div class="dpi-lvl">{lvl}</div>
      <div class="dpi-cap">去槓桿壓力指數 DPI</div>
    </div>
    <div class="hero-txt">
      <p>過去 6 個月維持率介於 <b>{min(maint):.0f}%–{max(maint):.0f}%</b>（斷頭線 130%），整體融資槓桿壓力
      <b>偏低</b>，DPI 期間最高僅 <b>{dpi_max:.0f}</b>。壓力最緊出現在 <b>3 月初的修正</b>與近期 <b>{last['date'][5:]}</b>。</p>
      <p class="warn">⚠️ 但融資餘額 6 個月 <b class="up">{bal_chg:+.0f}%</b>（{bal_yi[0]:,.0f}→{bal_yi[-1]:,.0f} 億）——槓桿水位大幅墊高、靠股價撐住維持率，潛在燃料在累積。</p>
    </div>
  </section>

  <section class="cards">
    <div class="card"><div class="k">融資餘額</div><div class="v">{bal_yi[-1]:,.0f}<span>億</span></div><div class="d {'up' if bal_chg>=0 else 'down'}">{bal_chg:+.1f}% / 6M</div></div>
    <div class="card"><div class="k">融資維持率</div><div class="v">{maint[-1]:.1f}<span>%</span></div><div class="d {'up' if maint_chg>=0 else 'down'}">{maint_chg:+.1f}pt / 6M</div></div>
    <div class="card"><div class="k">融券餘額</div><div class="v">{market[-1]['short_shares']:,}<span>張</span></div><div class="d">散戶放空</div></div>
    <div class="card"><div class="k">不限用途擔保品</div><div class="v">{bx_vals[-1]:,.0f}<span>億股</span></div><div class="d {'up' if bx_chg>=0 else 'down'}">{bx_chg:+.1f}% / 6M</div></div>
  </section>

  <section class="grid2">
    <div class="panel"><h2>去槓桿壓力指數 DPI（6 個月）</h2>{dpi_chart(dpi_rows)}
  ...
  <section class="panel">
    ...
    <p class="note">融資使用率＝餘額/限額（<span class="hot-t">≥40% 紅</span> 散戶擁擠）。空方看「借券賣出（法人）」遠大於「融券（散戶）」。Δ% 為 6 個月變化。</p>
  </section>
...
@media(max-width:720px){{.cards{{grid-template-columns:repeat(2,1fr)}}.grid2{{grid-template-columns:1fr}}}}
    ...
    print(f"✅ 產生 {OUT}（DPI 最新 {last['dpi']} {lvl}）")
